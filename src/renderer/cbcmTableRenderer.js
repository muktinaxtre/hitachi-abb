...
let dropDownData;
let menuObj = {}
ipcRenderer.send("loadDataCbcm")
ipcRenderer.on("loadDataCbcm", (e, arg) => {
...
let selectedId = []
ipcRenderer.on('changedCbcmDropDown', (e, arg) => {
    ...
    let dropdownArr = {
        boq_ref: arg.boq_ref,
        description_of_works: arg.description_of_works,
...
function loadPmDropDown(data) {
    ...
    let description_of_works_html = `<select name="description_of_works" id="description_of_works" multiple onchange="ocFilterClick('description_of_works')">`
    ...
    let description_of_works = data.description_of_works
    ...
    for (let i = 0; i < description_of_works.length; i++) {
        description_of_works_html = description_of_works_html + `<option value="` + description_of_works[i] + `">` + description_of_works[i] + `</option>`
    ...
    description_of_works_html = description_of_works_html + `</select>`
    ...
    $("#description_of_works-container").html(description_of_works_html)
    $("#description_of_works").multiselect(multiselectOption).multiselect('selectAll', false).multiselect('updateButtonText');
...
function ocFilterClick(id) {
    ...
    if (id == "description_of_works") {
        Object.assign(dataObj, { description_of_works: idArr })
    }
    if (id == "boq_sequence") {
    ...
    changeDropDownTimeout = setTimeout(() => {
        ...
        ipcRenderer.send('changeCbcmDropDown', arr)
    }, 2000)
...
function loadViDataTable(filter) {
    $('#cbcmData').DataTable({
        ...
        "order": [0, 'desc'],
        "ajax": {
            "url": "http://localhost:3456/getCbcmData",
            "type": "POST",
            "data": filter
        },
        "columns": [
