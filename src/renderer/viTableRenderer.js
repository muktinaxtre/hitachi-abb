...
let dropDownData;
let menuObj = {}
ipcRenderer.send("loadDataVi")
ipcRenderer.on("loadDataVi", (e, arg) => {

    dropDownData = arg.dropDownData
    loadViDropDown(dropDownData)

})

let selectedId = []
ipcRenderer.on('changedViDropDown', (e, arg) => {
    ...
    let allId = ["boq_ref", "boq_sequence", "date", "substation_name", "done_by", "work_order_no", "zone"]
    for (let i = 0; i < allId.length; i++) {
        let html = `<select name="` + allId[i] + `" id="` + allId[i] + `" multiple onchange="viFilterClick('` + allId[i] + `')">`
        let key = allId[i]
        let data = []
        let f_data = []
        let filteredData
...
function viFilterClick(id) {
    ...
    changeDropDownTimeout = setTimeout(() => {
        ...
        ipcRenderer.send('changeViDropDown', arr)
    }, 2000)
