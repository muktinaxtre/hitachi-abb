...
document.getElementById("mfForm").addEventListener('submit', (e) => {
    ...
    if (addNew) {
        ...
        ipcRenderer.send("removeAllPmData")
    } else {
        let data = {
            data: result,
            addNew: addNew
        ...
        ipcRenderer.send('updateAddPmData', data)
    }


})
ipcRenderer.on('completeAddPmData', (e, arg) => {
    ...
    console.log(arg)
    setTimeout(() => {
        ipcRenderer.send("loadNewView", "pmTable.ejs")
    }, 1000)
})
ipcRenderer.on("removedAllPmData", (e, arg) => {
