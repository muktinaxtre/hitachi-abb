...
let dropDownData;
let menuObj = {}
ipcRenderer.send("loadDataOc")
ipcRenderer.on("loadDataOc", (e, arg) => {
    dropDownData = arg.dropDownData
    loadOcDropDown(dropDownData)

})

let selectedId = []
ipcRenderer.on('changedOcDropDown', (e, arg) => {
...
function ocFilterClick(id) {
    ...
    changeDropDownTimeout = setTimeout(() => {
        ...
        ipcRenderer.send('changeOcDropDown', arr)
    }, 2000)
...
function loadViDataTable(filter) {
    $('#ocData').DataTable({
        ...
        "order": [0, 'desc'],
        "ajax": {
            "url": "http://localhost:3456/getOcData",
            "type": "POST",
            "data": filter
        },
        "columns": [
