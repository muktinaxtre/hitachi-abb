...
let dropDownData;
let menuObj = {}
ipcRenderer.send("loadDataSmh")
ipcRenderer.on("loadDataSmh", (e, arg) => {
    // console.log(arg)
...
let selectedId = []
ipcRenderer.on('changedSmhDropDown', (e, arg) => {
    // console.log(arg)
    let id = arg.id
    ...
    let dropdownArr = {
        ...
        boq_description: arg.boq_description,
        date: arg.date,
        substation_name: arg.substation_name,
        bay_no: arg.bay_no,
        voltage_ratings: arg.voltage_ratings,
        done_by: arg.done_by,
    ...
    // console.log(arg)
    let allId = ["boq_ref", "boq_sequence", "date", "substation_name", "bay_no", "voltage_ratings", "done_by", "work_order_no", "zone"]
    ...
    for (let i = 0; i < allId.length; i++) {
        ...
        if (key == "boq_description") {
            data = arg.boq_description
            f_data = menuObj.boq_description
            filteredData = f_data.filter(
                function (e) {
                    return this.indexOf(e) < 0;
                },
        ...
        if (key == "voltage_ratings") {
            data = arg.voltage_ratings
            f_data = menuObj.voltage_ratings
            filteredData = f_data.filter(
                function (e) {
                    return this.indexOf(e) < 0;
                },
        ...
        if (id !== key) {
            ...
            // $("#" + key).multiselect(multiselectOption).multiselect('selectAll', false).multiselect('updateButtonText');
...
function loadPmDropDown(data) {
    ...
    let boq_description_html = `<select name="boq_description" id="boq_description" multiple onchange="ocFilterClick('boq_description')">`
    ...
    let voltage_ratings_html = `<select name="voltage_ratings" id="voltage_ratings" multiple onchange="ocFilterClick('voltage_ratings')">`
    ...
    for (let i = 0; i < boq_description.length; i++) {
        boq_description_html = boq_description_html + `<option value="` + boq_description[i] + `">` + boq_description[i] + `</option>`
    ...
    for (let i = 0; i < voltage_ratings.length; i++) {
        voltage_ratings_html = voltage_ratings_html + `<option value="` + voltage_ratings[i] + `">` + voltage_ratings[i] + `</option>`
    ...
    $("#boq_description-container").html(boq_description_html)
    $("#boq_description").multiselect(multiselectOption).multiselect('selectAll', false).multiselect('updateButtonText');
    ...
    $("#voltage_ratings-container").html(voltage_ratings_html)
    $("#voltage_ratings").multiselect(multiselectOption).multiselect('selectAll', false).multiselect('updateButtonText');
...
function ocFilterClick(id) {
    ...
    if (id == "boq_description") {
        Object.assign(dataObj, { boq_description: idArr })
    }
    if (id == "date") {
    ...
    if (id == "voltage_ratings") {
        Object.assign(dataObj, { voltage_ratings: idArr })
    }
    if (id == "work_order_no") {
    ...
    changeDropDownTimeout = setTimeout(() => {
        ...
        ipcRenderer.send('changeSmhDropDown', arr)
    }, 2000)
...
function loadViDataTable(filter) {
    $('#smhData').DataTable({
        ...
        "order": [0, 'desc'],
        "ajax": {
            "url": "http://localhost:3456/getSmhData",
            "type": "POST",
            "data": filter
        },
        "columns": [
            ...
            { "data": "boq_ref" },
            { "data": "boq_description" },
            { "data": "date" },
            { "data": "boq_sequence" },
            { "data": "substation_name" },
            { "data": "bay_no" },
            { "data": "voltage_ratings" },
            { "data": "done_by" },
            { "data": "zone" },
