...
ipcMain.on("loadDataOc", (e, arg) => {
    ocData.find({}).exec((error, document) => {
        if (error) {
            console.log(error)
        } else {
            let boq_ref = []
...
ipcMain.on('changeOcDropDown', (e, args) => {
    ...
    ocData.find(filter).exec((err, document) => {
        ...
        e.sender.send('changedOcDropDown', dropDownData)
